...
if __name__ == "__main__":
    ...
    features_3d = tsne.fit_transform(all_features_scaled)
    ...
    for idx, (label_name, color) in enumerate(zip(label_dict.keys(), colors)):
        ...
        ax.scatter(features_3d[indices, 0], features_3d[indices, 1], features_3d[indices, 2], c=[color],
                   label=label_name, s=20)

    ax.set_title("特征分布三维可视化 (t-SNE)")
    ax.set_xlabel("t-SNE 第一主成分")
    ax.set_ylabel("t-SNE 第二主成分")
    ax.set_zlabel("t-SNE 第三主成分")
    ax.legend(title="真实标签")
    plt.tight_layout()
    plt.savefig("logs/feature_distribution_t_sne_3d.png", dpi=600, bbox_inches='tight')
    plt.show()
